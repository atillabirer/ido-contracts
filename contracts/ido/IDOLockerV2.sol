...
contract IDOLockerV2 is Ownable, ReentrancyGuard {
  ...
  struct XStellaTiers {
    uint256 minHolding;
		uint256 multiplier; // in 100,000 Demonition
  }

  PoolInfo[] public poolInfo;
  mapping(uint256 => mapping(address => UserInfo)) public userInfo;
  mapping(uint256 => XStellaTiers[]) public tiers;
  ...
  function addTier(uint256 _pid, XStellaTiers memory _tier) external onlyOwner {
    tiers[_pid].push(_tier);
  }

  function removeTier(uint256 _pid, uint256 index) external onlyOwner {
    delete tiers[_pid][index];
  }

  function editTier(uint256 _pid, uint256 index, XStellaTiers memory _tier)
    external
    onlyOwner
  {
    tiers[_pid][index] = _tier;
  }

  function getUserTier(uint256 _pid, address _userAddr) external view returns (uint256 multiplier) {
		UserInfo storage user = userInfo[_pid][_userAddr];
    for (uint8 index = 0; index < tiers[_pid].length; index++) {
	    XStellaTiers memory _tier = tiers[_pid][index];
	    if (user.amount >= _tier.minHolding) {
	      multiplier = _tier.multiplier;
	    }
		}
  }
}
